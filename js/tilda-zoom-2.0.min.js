...
function t_zoomHandler() {
    ...
    var id = $(this).closest(".r").attr("id"),
        parentBlock = $("#" + id),
        images = parentBlock.find(".t-zoomable:not(.t-slds__thumbs_gallery):not(.tn-atom__slds-img)");
    if ($("#" + id).find(".t-slds").length) {
        var slider = $(this).closest(".t-slds");
        slider.length && (images = slider.find(".t-zoomable:not(.t-slds__thumbs_gallery)"));
    }
    ...
    images.each(function (i, image) {
        var imgtitle = "",
            ...
            titlebody = "",
            descrbody = "",
            images_urls = $(image).attr("data-img-zoom-url").split(",");
        ($(image).is("img") || $(image).is("div")) && ((imgtitle = $(image).attr("title") || ""), (imgdescr = $(image).attr("data-img-zoom-descr") || "")),
            "" !== imgtitle && (titlebody = '<div class="t-zoomer__title t-name t-descr_xxs">' + imgtitle + "</div>"),
            "" !== imgdescr && (descrbody = '<div class="t-zoomer__descr t-descr t-descr_xxs">' + imgdescr + "</div>"),
            sliderTrack.append(
                '<div class="t-carousel__zoomer__item"><div class="t-carousel__zoomer__wrapper"><img class="t-carousel__zoomer__img" src="' + images_urls + '"></div><div class="t-zoomer__comments">' + titlebody + descrbody + "</div></div>"
            );
    }),
    ...
    zoomerComments.each(function (i, comment) {
        var zoomerTitle = $(comment).find(".t-zoomer__title"),
            zoomerDescr = $(comment).find(".t-zoomer__descr");
        (("" === zoomerTitle && "" === zoomerDescr) || (!zoomerTitle.length && !zoomerDescr.length)) && $(comment).css("padding", "0");
    ...
    if ((zoomedImages.css("max-height", "calc(100vh - " + (maxCommentsHeight + imageBordersWidth) + "px"), -1 !== zoomedImages.attr("src").indexOf(".svg"))) {
        var winWidth = $(window).width();
        zoomedImages.css("width", winWidth);
    }
    arrowWrappers.css("top", "calc(50% - " + maxCommentsHeight / 2 + "px)");
    var target_url = $(this).attr("data-img-zoom-url"),
        target_img,
        targetItem = $('.t-carousel__zoomer__img[src="' + target_url + '"]').closest(".t-carousel__zoomer__item");
    if (
        (slideItems.each(function (index, item) {
            $(item).attr("data-zoomer-slide-number", index);
        }),
        slideItems.length > 1 && t_zoom_loopSlider(),
        void 0 !== targetItem.position())
    ) {
        var targetPosition = targetItem.position().left;
        targetItem.addClass("active"), sliderTrack.css({ transition: "none", transform: "translateX(" + -targetPosition + "px)" });
    }
    setTimeout(function () {
        sliderTrack.css("transition", "");
    }),
        $(".t-carousel__zoomer__control_right").click(function () {
            "y" !== sliderTrack.attr("data-on-transition") &&
                "y" !== modal.attr("data-on-drag") &&
                (t_zoom_unscale(),
                setTimeout(function () {
        ...
        $(document).keydown(function (e) {
            ...
            27 == e.keyCode) &&
                (t_zoom_close(),
                $(document).find(".t-popup_show").length > 0 &&
                    $(document).keydown(function (e) {
    ...
    $("body").addClass("t-zoomer__show_fixed"),
        1 == slides_count && $(".t-carousel__zoomer__control").css("display", "none"),
        ...
        window.isMobile && (t_zoom_setHideControlsTimer(), modal.on("touchstart touchmove touchend mousemove", t_zoom_setHideControlsTimer));
}
function t_zoom_initSwipe() {
    ...
    if (slideItems.length > 1) {
        var hammer = new Hammer($(".t-zoomer__wrapper")[0], { domEvents: !0, inputClass: Hammer.TouchInput, cssProps: { touchCollout: "default" }, recognizers: [[Hammer.Pan, { direction: Hammer.DIRECTION_HORIZONTAL }]] }),
            sliderTrackPosition,
            isScaled = !1;
        hammer.on("panstart", function () {
            "y" !== sliderTrack.attr("data-on-transition") ? ((sliderTrackPosition = sliderTrack.position().left), sliderTrack.css("transition", "none")) : (sliderTrackPosition = null),
                (isScaled = 1 !== window.visualViewport.scale || modal.hasClass("scale-active"));
        }),
            hammer.on("panmove", function (event) {
                ...
                if (
                    ...
                    1 === event.maxPointers &&
                    !isScaled &&
                    (Math.abs(event.deltaX) > 40 && sliderTrack.attr("data-on-drag", "y"), sliderTrackPosition)
                ) {
                    var newTrackPosition = sliderTrackPosition + event.deltaX;
                    sliderTrack.css("transform", "translateX(" + newTrackPosition + "px)");
                }
            }),
            window.tzoomopenonce ||
                hammer.on("panend", function (event) {
                    var sliderTrack = $(".t-carousel__zoomer__track");
                    if ((sliderTrack.attr("data-on-drag", ""), "y" !== sliderTrack.attr("data-on-transition") && "y" !== modal.attr("data-on-drag") && 1 === event.maxPointers && !isScaled)) {
                        sliderTrack.css("transition", "");
                        var velocity = Math.abs(event.velocityX),
                            sliderTrackOffset = sliderTrack.position().left,
                            slideItem,
                            slideWidth = $(".t-carousel__zoomer__item").width(),
                            targetSlideOffset = sliderTrack.find(".t-carousel__zoomer__item.active").position().left,
                            distance,
                            transitionTime = (slideWidth - Math.abs(sliderTrackOffset + targetSlideOffset)) / velocity / 1e3;
...
function t_zoom_showSlide(direction) {
    var sliderTrack = $(".t-carousel__zoomer__track"),
        ...
        targetItem,
        currentSlideIndex = sliderTrack.find(".t-carousel__zoomer__item.active").index();
    "next" === direction && ((currentSlideIndex = (currentSlideIndex + 1) % slideItems.length), sliderTrack.attr("data-on-transition", "y")),
        "prev" === direction && ((currentSlideIndex = (slideItems.length + (currentSlideIndex - 1)) % slideItems.length), sliderTrack.attr("data-on-transition", "y"));
    var trackPosition = slideItems.eq(currentSlideIndex).position().left;
    slideItems.removeClass("active").eq(currentSlideIndex).addClass("active"), sliderTrack.css("transform", "translateX(" + -trackPosition + "px");
}
function t_zoom_transitForLoop(side) {
    ...
    "start" === side && (currentSlideIndex = $slideItems.length - 2),
        ...
        $sliderTrack.css({ transition: "none", transform: "translateX(" + -slideOffset + "px)" }),
        setTimeout(function () {
            $sliderTrack.css("transition", "");
        });
}
function t_zoom_loopSlider() {
    ...
    sliderTrack.on("transitionend webkitTransitionEnd oTransitionEnd", function () {
        var currentSlideIndex = sliderTrack.find(".active").index();
        0 === currentSlideIndex && t_zoom_transitForLoop("start"), currentSlideIndex === slidesCount - 1 && t_zoom_transitForLoop("end"), sliderTrack.attr("data-on-transition", "");
    });
}
function t_zoom_initCloseSwipe() {
    var hammer = new Hammer($(".t-zoomer__wrapper")[0], { domEvents: !0, inputClass: Hammer.TouchInput, cssProps: { touchCollout: "default" }, recognizers: [[Hammer.Pan, { direction: Hammer.DIRECTION_VERTICAL }]] }),
        modal = $(".t-zoomer__wrapper"),
        sliderTrack = $(".t-carousel__zoomer__track"),
        isScaled = !1,
        modalPosition;
    hammer.on("panstart", function () {
        (modalPosition = modal.position().top), modal.css("transition", "none"), (isScaled = 1 !== window.visualViewport.scale || modal.hasClass("scale-active"));
    }),
        hammer.on("panmove", function (event) {
            var deltaY = Math.abs(event.deltaY);
            if (("y" !== sliderTrack.attr("data-on-drag") || "y" === modal.attr("data-on-drag")) && ((event.angle > -120 && event.angle < -60) || (event.angle < 120 && event.angle > 60)) && 1 === event.maxPointers && !isScaled) {
                deltaY > 40 && modal.attr("data-on-drag", "y");
                var newTrackPosition = modalPosition + event.deltaY;
                modal.css("transform", "translateY(" + newTrackPosition + "px)");
            }
        }),
...
function t_zoom_closeSwipeHandler(event) {
    var modal = $(".t-zoomer__wrapper"),
        ...
        isScaled = 1 !== window.visualViewport.scale || modal.hasClass("scale-active");
    modal.css("transition", "transform 300ms ease-out"),
        Math.abs(event.deltaY) < 40 && modal.css("transform", ""),
        "y" !== modal.attr("data-on-drag") ||
            1 !== event.maxPointers ||
            isScaled ||
            (event.deltaY < -200 || event.velocityY < -0.3
                ? (modal.css("transform", "translateY(-100vh)"),
                  setTimeout(function () {
                      t_zoom_close(), modal.css("transform", "");
                  }, 300))
                : event.deltaY > 200 || event.velocityY > 0.3
                ? (modal.css("transform", "translateY(100vh)"),
                  setTimeout(function () {
...
function t_zoom_checkForScale() {
    var eventAdded = !1,
        zoomedImage = $(".t-carousel__zoomer__item.active .t-carousel__zoomer__img:not(.loaded)");
    if (
        zoomedImage.length &&
        (zoomedImage.load(function () {
            if (!eventAdded)
                return $(window).width() < zoomedImage[0].naturalWidth || $(window).height() < zoomedImage[0].naturalHeight
                    ? (zoomedImage.addClass("loaded"), window.isDoubletapScaleAdded || t_zoom_doubletapScaleInit(), void t_zoom_scale_init())
                    : void 0;
        }),
        zoomedImage[0].complete && !eventAdded)
    ) {
        if (((eventAdded = !0), $(window).width() < zoomedImage[0].naturalWidth || $(window).height() < zoomedImage[0].naturalHeight)) return window.isDoubletapScaleAdded || t_zoom_doubletapScaleInit(), void t_zoom_scale_init();
        ...
        "y" !== $(".t-zoomer__scale").attr("data-zoom-scale-init") &&
            ...
            $(".t-zoomer__wrapper").on("click", ".t-zoomer__scale", function (event) {
                ...
                var zoomedImage = $(".t-carousel__zoomer__item.active .t-carousel__zoomer__img"),
                    zoomedWrapper = $(".t-zoomer__wrapper"),
                    zoomerInner = $(".t-carousel__zoomer__inner"),
                    zoomerTrack = $(".t-carousel__zoomer__track");
                zoomedImage.css("max-height", ""),
                    zoomerTrack.css({ transition: "none", transform: "none" }),
                    zoomedWrapper.hasClass("scale-active")
                        ? t_zoom_unscale()
                        : (zoomedWrapper.addClass("scale-active"), zoomerInner.addClass("scale-active"), window.isMobile ? t_zoom_mobileZoomPositioningInit(zoomedImage) : t_zoom_desktopZoomPositioningInit(zoomedImage, event));
            }));
}
function t_zoom_doubletapScaleInit() {
    var hammer;
    (window.isDoubletapScaleAdded = !0),
        new Hammer($(".t-zoomer__wrapper")[0], { domEvents: !0, inputClass: Hammer.TouchInput, cssProps: { touchCollout: "default" }, recognizers: [[Hammer.Tap]] }).on("tap", function (e) {
            if (2 === e.tapCount && $("body").hasClass("t-zoomer__show") && !e.target.closest(".t-carousel__zoomer__control")) {
                var zoomedImage = $(".t-carousel__zoomer__item.active .t-carousel__zoomer__img"),
                    zoomedWrapper = $(".t-zoomer__wrapper"),
                    zoomerInner = $(".t-carousel__zoomer__inner"),
                    zoomerTrack = $(".t-carousel__zoomer__track");
                zoomedImage.css("max-height", ""),
                    zoomerTrack.css({ transition: "none", transform: "none" }),
                    zoomedWrapper.hasClass("scale-active") ? t_zoom_unscale() : (zoomedWrapper.addClass("scale-active"), zoomerInner.addClass("scale-active"), t_zoom_mobileZoomPositioningInit(zoomedImage));
            }
        });
}
function t_zoom_desktopZoomPositioningInit(zoomedImage, event) {
    var leftCoordinate = ($(window).width() - zoomedImage.width()) / 2,
        ...
        clientXpercent,
        imageXpx;
    zoomedImage.css({ left: leftCoordinate, top: topCoordinate }),
        $(window).width() < zoomedImage[0].naturalWidth &&
            ((clientXpercent = (100 * event.clientX) / $(window).width()),
            ...
            zoomedImage.on(window.isMobile ? "touchmove" : "mousemove", function (e) {
                (clientXpercent = (100 * (void 0 !== e.originalEvent.touches ? e.originalEvent.touches[0].clientX : e.clientX)) / $(window).width()),
                    (imageXpx = (-clientXpercent * (zoomedImage.width() - $(window).width())) / 100),
                    zoomedImage.css("left", imageXpx + "px");
            })),
        $(window).height() < zoomedImage[0].naturalHeight &&
            ((clientYpercent = (100 * event.clientY) / $(window).height()),
            (imageYpx = (-clientYpercent * (zoomedImage.height() - $(window).height())) / 100),
            zoomedImage.css("top", imageYpx + "px"),
            zoomedImage.on(window.isMobile ? "touchmove" : "mousemove", function (e) {
                (clientYpercent = (100 * (void 0 !== e.originalEvent.touches ? e.originalEvent.touches[0].clientY : e.clientY)) / $(window).height()),
                    (imageYpx = (-clientYpercent * (zoomedImage.height() - $(window).height())) / 100),
                    zoomedImage.css("top", imageYpx + "px");
            }));
}
    ...
    zoomedImage.on("touchstart", function (event) {
        ...
        zoomedImage.on("touchmove", function (event) {
            ...
            (currentTranslate.x = currentPosition.x + imageOffsetX),
                ...
                currentTranslate.x > -leftCoordinate && (currentTranslate.x = -leftCoordinate),
                currentTranslate.x < leftCoordinate && (currentTranslate.x = leftCoordinate),
                currentTranslate.y > -topCoordinate && (currentTranslate.y = -topCoordinate),
                currentTranslate.y < topCoordinate && (currentTranslate.y = topCoordinate),
        ...
        zoomedImage.on("touchend touchcancel", function () {
            (currentPosition.x = currentTranslate.x), (currentPosition.y = currentTranslate.y);
        });
}
function t_zoom_getTouchEventXY(event) {
    var coordinates = { x: 0, y: 0 };
    if ("touchstart" == event.type || "touchmove" == event.type || "touchend" == event.type || "touchcancel" == event.type) {
...
function t_zoom_unscale() {
    ...
    var zoomedItem = $(".t-carousel__zoomer__item.active"),
        ...
        zoomerTrack = $(".t-carousel__zoomer__track"),
        modal,
        imageBordersWidth = $(".t-zoomer__wrapper").height() - zoomedItem.height(),
        zoomerComments,
        height = $(zoomedItem).find(".t-zoomer__comments").innerHeight();
    if ((zoomedImage.off("mousemove touchmove").css({ transform: "", left: "", top: "", maxHeight: "calc(100vh - " + (imageBordersWidth + height) + "px" }), void 0 !== zoomedItem.position())) {
...
function t_zoom_lockScroll() {
    ...
    if (((window.isiOS && !window.MSStream) || isAndroid) && (("" !== window.isiOSVersion && void 0 !== window.isiOSVersion) || isAndroid) && (11 === window.isiOSVersion[0] || isAndroid)) {
        var body = $("body");
    ...
    "function" == typeof window[funcName]
        ...
        : setTimeout(function checkFuncExist() {
              if ("function" != typeof window[funcName]) {
                  if ("complete" === document.readyState && "function" != typeof window[funcName]) throw new Error(funcName + " is undefined");
...
function t_zoom_setModalColor(parent) {
    var COLOR_WHITE = "#ffffff",
        COLOR_BLACK = "#000000",
        parentBGColor = parent.attr("data-bg-color") ? parent.attr("data-bg-color") : "#ffffff";
    ...
    var modalContainer = $(".t-zoomer__container"),
        ...
        modalTextColor,
        controlsBGColor,
        modalBGColor = "black" === t_zoom_luma_rgb(parentBGColor) ? "#000000" : "#ffffff";
    "#000000" === modalBGColor
        ? ((modalTextColor = "#ffffff"), (controlsBGColor = "rgba(1, 1, 1, 0.3)"), arrowsWrapper.addClass("t-carousel__zoomer__arrow__wrapper_dark"))
        : ((modalTextColor = "#000000"), (controlsBGColor = "rgba(255, 255, 255, 0.3)"), arrowsWrapper.removeClass("t-carousel__zoomer__arrow__wrapper_dark")),
        controlsBG.css("background", controlsBGColor),
        modalContainer.css({ "background-color": modalBGColor, color: modalTextColor }),
        modalSvg.each(function (_, svg) {
            $(svg).attr("fill");
        }),
        modalPath.each(function (_, path) {
            $(path).attr("stroke") && $(path).attr("stroke", modalTextColor), $(path).attr("fill") && $(path).attr("fill", modalTextColor);
        }),
...
function t_zoom_luma_rgb(color) {
    ...
    return rgb.length < 3 ? "black" : 0.2126 * rgb[0] + 0.7152 * rgb[1] + 0.0722 * rgb[2] < 128 ? "black" : "white";
}
function t_zoom_hexToRgb(hex) {
    ...
    var result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex),
        aaa = result ? { r: parseInt(result[1], 16), g: parseInt(result[2], 16), b: parseInt(result[3], 16) } : null;
